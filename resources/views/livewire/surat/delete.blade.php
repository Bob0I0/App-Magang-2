<div x-cloak x-data="{ modalIsOpen: false }" class="inline-block">
    ...
    <div x-show="modalIsOpen" x-transition.opacity.duration.200ms x-trap.inert.noscroll="modalIsOpen" x-on:keydown.esc.window="modalIsOpen = false" x-on:click.self="modalIsOpen = false" class="fixed inset-0 z-30 flex items-end justify-center bg-black/20 p-4 pb-8 backdrop-blur-md sm:items-center lg:p-8" role="dialog" aria-modal="true" aria-labelledby="deleteModalTitle-{{ $surat_id }}">
            ...
            <div class="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                <div class="sm:flex sm:items-start">
                    ...
                    <div class="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
                        <h3 class="text-base font-semibold leading-6 text-gray-900" id="deleteModalTitle-{{ $surat_id }}">Hapus Surat</h3>
                        <div class="mt-2">
                            <p class="text-sm text-gray-500">Apakah anda yakin ingin menghapus surat dengan nomor <span class="font-bold">{{ $nomor_surat }}</span>? Data yang dihapus tidak dapat dikembalikan.</p>
                        </div>
                    </div>
                </div>
            </div>
